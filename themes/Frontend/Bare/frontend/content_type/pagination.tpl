    ...
    <div class="listing--paging panel--paging">
        ...
        {block name="frontend_listing_actions_paging_first"}
            {if $sPage > 1}
                ...
                <a href="{$baseUrl}?p=1" title="{$snippetListingLinkFirst|escape}" class="paging--link paging--prev" data-action-link="true">
        ...
        {block name='frontend_listing_actions_paging_previous'}
            {if $sPage > 1}
                ...
                <a href="{$baseUrl}?p={$sPage-1}" title="{$snippetListingLinkPrevious|escape}" class="paging--link paging--prev" data-action-link="true">
                    <i class="icon--arrow-left"></i>
                </a>
            {/if}
        {/block}
        ...
        {block name='frontend_listing_actions_paging_numbers'}
            {if $pages > 1}
                <a title="{$sType->getName()|escape}" class="paging--link is--active">{$sPage}</a>
            {/if}
        {/block}
        ...
        {block name='frontend_listing_actions_paging_next'}
            {if $sPage < $pages}
                ...
                <a href="{$baseUrl}?p={$sPage+1}" title="{$snippetListingLinkNext|escape}" class="paging--link paging--next" data-action-link="true">
        ...
        {block name="frontend_listing_actions_paging_last"}
            {if $sPage < $pages}
                ...
                <a href="{$baseUrl}?p={$pages}" title="{$snippetListingLinkLast|escape}" class="paging--link paging--next" data-action-link="true">
                    <i class="icon--arrow-right"></i>
                    <i class="icon--arrow-right"></i>
                </a>
            {/if}
