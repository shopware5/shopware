...
class NestedTreeRepository extends AbstractTreeRepository
{
    ...
    public function buildTree(array $nodes, array $options = array())
    {
        ...
        $meta = $this->getClassMetadata();
        $config = $this->listener->getConfiguration($this->_em, $meta->name);
        $childrenField = isset($options['childrenField']) ? $options['childrenField'] : '__children';
        $nestedTree = array();
        $l = 0;
        ...
        if (count($nodes) > 0) {
            ...
            foreach ($nodes as $child) {
                ...
                if ($l == 0) {
                    ...
                    // Add child to parent
                    $i = count($stack[$l - 1][$childrenField]);
                    $stack[$l - 1][$childrenField][$i] = $item;
                    $stack[] = &$stack[$l - 1][$childrenField][$i];
                }
            }
        }
        ...
        $build = function($tree) use (&$build, &$options, $childrenField) {
            ...
            foreach ($tree as $node) {
                ...
                if (count($node[$childrenField]) > 0) {
                    $output .= $build($node[$childrenField]);
                }
