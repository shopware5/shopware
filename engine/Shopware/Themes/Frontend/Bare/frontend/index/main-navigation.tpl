<ul class="navigation--list container" role="menubar" itemscope="itemscope" itemtype="http://schema.org/SiteNavigationElement">
    ...
    <li class="navigation--entry {if $sCategoryCurrent eq $sCategoryStart} is--active{/if}" role="menuitem">
        <a class="navigation--link is--first{if $sCategoryCurrent eq $sCategoryStart} active{/if}" href="{url controller='index'}" title="{"{s name='IndexLinkHome' namespace="frontend/index/categories_top"}{/s}"|escape}" itemprop="url">
		...
		{block name='frontend_index_navigation_categories_top_entry'}
			{if !$sCategory.hidetop}
				<li class="navigation--entry{if $sCategory.flag} is--active{/if}" role="menuitem">
					<a class="navigation--link{if $sCategory.flag} is--active{/if}" href="{$sCategory.link}" title="{$sCategory.description|escape}" itemprop="url">
