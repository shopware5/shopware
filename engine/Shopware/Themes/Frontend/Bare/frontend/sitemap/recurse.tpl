...
{foreach from=$sCategoryTree item=categoryTree}
	...
	<ul class="sitemap--navigation-entry list--unstyled">
            <li>
		<a href="{$categoryTree.link}" title="{$categoryTree.name}" class="sitemap--navigation-link is--active">{$categoryTree.name}</a>
    ...
    {else}
		...
		<a href="{$categoryTree.link}" title="{$categoryTree.name}" class="sitemap--navigation-link">{$categoryTree.name}</a></li>{if $depth==1}</ul>{/if}
    {/if}
{/foreach}
