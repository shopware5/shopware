		...
		{block name="frontend_custom_content"}
			<div class="custom-page--container" data-tab-content="true" data-mode="remote">
										...
										{block name="frontend_custom_tab_navigation_entry"}
											<li class="navigation--entry">
												...
												{block name="frontend_custom_tab_navigation_link"}
													<a class="navigation--link{if $subPage.active} is--active{/if}" href="{url controller=custom sCustom=$subPage.id}" title="{$subPage.description|escape}">
														{$subPage.description}
													</a>
												{/block}
											</li>
				...
				{block name="frontend_custom_article"}
					<div class="tabs--content-container tab--content panel has--border is--rounded{if !is_array($sCustomPage.parent)} is--active-parent{/if}">
