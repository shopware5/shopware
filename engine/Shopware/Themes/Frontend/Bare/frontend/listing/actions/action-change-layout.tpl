...
{if !$sCategoryContent.noViewSelect}
    ...
    <div class="action--change-layout action--content block">
        ...
        {block name="frontend_listing_actions_change_layout_link_table"}
            <a href="{url params=$templateLinks sViewport='cat' sCategory=$sCategoryContent.id sPage=1 sTemplate='table'}" class="action--link link--table-view {if $sBoxMode=='table'}is--active{/if}" title="{s name='ListingActionsSettingsTable'}Tabellen-Ansicht{/s}">
        ...
        {block name="frontend_listing_actions_change_layout_link_list"}
            <a href="{url params=$templateLinks sViewport='cat' sCategory=$sCategoryContent.id sPage=1 sTemplate='list'}" class="action--link link--list-view {if $sBoxMode=='list'}is--active{/if}" title="{s name='ListingActionsSettingsList'}Listen-Ansicht{/s}">
