 ...
 * add and edit orders.
 */
class Shopware_Controllers_Backend_OrderPayone extends Shopware_Controllers_Backend_ExtJs
{
    /**
...
//        $this->addAclPermission('deleteAction', 'delete', $namespace->get('no_delete_rights', 'You do not have sufficient rights to delete a customers.'));
    ...
    /**
     * Get a list of available order status
     */
    public function getOrderStatusAction() {
        ...
        $query = $this->getOrderStatusQuery($filter, $sort, $offset, $limit);
        $total = Shopware()->Models()->getQueryCount($query);
        $orderStatus = $query->getArrayResult();
    ...
    public function getListAction()
    {
        try {
            ...
            $payonePaymentMean = (int)$this->getPayonePaymentmean();
            
            $forcePayment = array('property' => 'orders.paymentId', 'value' => $payonePaymentMean);
            $cleanFilter = array();
            foreach($filter as $f) {
                if($f['property'] != 'orders.paymentId') {
                    array_push($cleanFilter, $f);
                }
            }
            array_push($cleanFilter, $forcePayment);
            $filter = $cleanFilter;
            if (empty($sort)) {
     ...
     * @return Array
     */
    public function getPayonePaymentmean()
    {
        $sql= "SELECT `id`  FROM s_core_paymentmeans p WHERE p.name LIKE '%payone%'";
    ...
    {
        $data = $this->getRepository()->getDetailStatusQuery()->getArrayResult();
        $this->View()->assign(array('success' => true, 'data' => $data));
    }
    ...
    private function getOrder($id) {
        $query = $this->getRepository()->getOrdersQuery(array(array('orders.id' => $id)), array());
    ...
    public function batchProcessAction()
    {
        ...
        foreach($orders as $key => $data) {
            ...
            $order->setOrderStatus(
                Shopware()->Models()->find('Shopware\Models\Order\Status', $data['status'])
            );
            $order->setPaymentStatus(
                Shopware()->Models()->find('Shopware\Models\Order\Status', $data['cleared'])
            );
    ...
    private function mergeDocuments($paths) {
        include_once(Shopware()->OldPath()."engine/vendor/fpdf/fpdf.php");
        include_once(Shopware()->OldPath()."engine/vendor/fpdf/fpdi.php");
    ...
    public function createDocumentAction()
    {
       ...
       if (!empty($orderId) && !empty($documentType)) {
        ...
        try {
            ...
            $order = $this->getRepository()->getOrdersQuery(array(array('orders.id' => $id)))->getArrayResult();
    ...
    private function createDocument($orderId, $documentType) {
        $document = Shopware_Components_Document::initDocument(
            ...
            array(
                ...
                'date'                    => $this->Request()->getParam('displayDate', null),
                'delivery_date'           => $this->Request()->getParam('deliveryDate', null),
    ...
    private function getPositionAssociatedData($data) {
        ...
        if (!empty($data['articleNumber'])) {
            $articleRepository = Shopware()->Models()->getRepository('Shopware\Models\Article\Detail');

            /**@var $article Shopware\Models\Article\Detail*/
            $article = $articleRepository->findOneBy(array('number' => $data['articleNumber']));
