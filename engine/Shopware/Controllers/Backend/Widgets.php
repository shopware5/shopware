...
class Shopware_Controllers_Backend_Widgets extends Shopware_Controllers_Backend_ExtJs
{
    ...
    public function getListAction()
    {
        ...
        foreach ($data as &$widgetData) {
            ...
            $widgetData['label'] = $this->container->get('snippets')->getNamespace('backend/widget/labels')
    ...
    {
        $model = $this->container->get('models')->find('Shopware\Models\Widget\View', $viewId);
        $model->setPosition($position);
        $model->setColumn($column);

        $this->container->get('models')->persist($model);
    ...
    public function addWidgetViewAction()
    {
        ...
        $model->setWidget(
            $this->container->get('models')->find('Shopware\Models\Widget\Widget', $widgetId)
        );
        $model->setAuth(
            $this->container->get('models')->find('Shopware\Models\User\User', $userID)
        );
        $model->setColumn($column);
        $model->setPosition($position);
    ...
    public function removeWidgetViewAction()
    {
        ...
        $model = $this->container->get('models')->find('Shopware\Models\Widget\View', $id);
        $this->container->get('models')->remove($model);
        $this->container->get('models')->flush();
    ...
    {
        // Get turnovers
        $fetchAmount = $this->container->get('db')->fetchRow(
            "SELECT
                (
        ...
        // Get visitors
        $fetchVisitors = $this->container->get('db')->fetchRow(
            "SELECT
                (
        ...
        // Get new customers
        $fetchCustomers = $this->container->get('db')->fetchRow(
            "SELECT
                (
        ...
        // Get order-count
        $fetchOrders = $this->container->get('db')->fetchRow(
            "SELECT
                (
        ...
        ";
        $fetchConversion = $this->container->get('db')->fetchRow(
            $sql,
        ...
        $namespace = $this->container->get('snippets')->getNamespace('backend/widget/controller');
        $this->View()->assign(
            array(
                'success' => true,
                'data' => array(
    ...
    public function getVisitorsAction()
    {
        ...
        $data = $this->container->get('db')->fetchAll($sql, array($timeBack));
        ...
        $currentUsers = $this->container->get('db')->fetchOne(
        ...
        $fetchLoggedInUsers = $this->container->get('db')->fetchAll("
            SELECT s.userID,
    ...
    public function getLastOrdersAction()
    {
        ...
        if (!empty($subshopID)) {
            $addSqlSubshop = "
            AND s_order.subshopID = " . $this->container->get('db')->quote($subshopID);
        }

        if (!empty($restrictPayment)) {
            $addSqlPayment = "
            AND s_order.paymentID = " . $this->container->get('db')->quote($restrictPayment);
        }
    ...
    public function getNoticeAction()
    {
        ...
        $noticeMsg = $this->container->get('db')->fetchOne(
            "
    ...
    public function saveNoticeAction()
    {
        ...
        if ($this->container->get('db')->fetchOne("SELECT id FROM s_plugin_widgets_notes WHERE userID = ?", array($userID))) {
            // Update
    ...
    public function getLastMerchantAction()
    {
        ...
        $fetchUsersToUnlock = $this->container->get('db')->fetchAll($sql);
