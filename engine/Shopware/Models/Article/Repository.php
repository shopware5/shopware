...
class Repository extends ModelRepository
{
    ...
    /**
     * Returns an instance of the \Doctrine\ORM\Query object which selects all ESD by the given esd ids list
     *
     * @param array $esdListingIds
     * @return \Doctrine\ORM\Query
     */
    public function getEsdAttributesQuery($esdListingIds)
    {
        $builder = $this->getEsdAttributesQueryBuilder($esdListingIds);

        return $builder->getQuery();
    }

    /**
     * Helper function to create the query builder for the "getEsdAttributesQuery" function.
     * This function can be hooked to modify the query builder of the query object.
     *
     * @param array $esdListingIds
     * @return \Doctrine\ORM\QueryBuilder
     */
    protected function getEsdAttributesQueryBuilder($esdListingIds)
    {
        $builder = $this->getEntityManager()->createQueryBuilder();
        ...
        $builder->select('esd', 'attribute')
                ->from('Shopware\Models\Article\Esd', 'esd')
                ->leftJoin('esd.attribute', 'attribute')
                ->where('esd.id IN (:esdIds)')
                ->setParameter('esdIds', $esdListingIds);
     ...
     * @return \Doctrine\ORM\Query
     */
    public function getEsdIdsByArticleIdQuery($articleId)
    {
        $builder = $this->getEsdIdsByArticleIdQueryBuilder($articleId);

        return $builder->getQuery();
    }

    /**
     * Helper function to create the query builder for the "getEsdIdsByArticleIdQuery" function.
     * This function can be hooked to modify the query builder of the query object.
     *
     * @param int $articleId
     * @return \Doctrine\ORM\QueryBuilder
     */
    protected function getEsdIdsByArticleIdQueryBuilder($articleId)
    {
        $builder = $this->getEntityManager()->createQueryBuilder();

        $builder->select('esd.id')
                ->from('Shopware\Models\Article\Esd', 'esd')
                ->leftJoin('esd.article', 'article')
                ->where('article.id = :articleId')
                ->setParameter('articleId', $articleId);
