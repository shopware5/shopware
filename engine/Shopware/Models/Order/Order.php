...
class Order extends ModelEntity
{
    ...
    public function calculateInvoiceAmount()
    {
        ...
        foreach ($this->getDetails() as $detail) {
            ...
            if ($this->net) {
                $invoiceAmountNet += round(round($detail->getPrice(), 2) / 100 * (100 + $taxValue), 2) * $detail->getQuantity();
            } else {
                $invoiceAmountNet += round(round($detail->getPrice(), 2) / (100 + $taxValue) * 100, 2) * $detail->getQuantity();
            }
        }
