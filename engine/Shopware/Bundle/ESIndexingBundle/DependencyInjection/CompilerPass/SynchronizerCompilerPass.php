...
class SynchronizerCompilerPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->hasDefinition('shopware_elastic_search.composite_synchronizer_factory')) {
            return;
        }

        $definition = $container->getDefinition('shopware_elastic_search.composite_synchronizer_factory');
        $this->replaceArgument($container, $definition, 'shopware_elastic_search.synchronizer', 0);
    }
