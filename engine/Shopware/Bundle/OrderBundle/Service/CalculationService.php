...
class CalculationService implements CalculationServiceInterface
{
    ...
    {
        if($order->getDetails() && count($order->getDetails()) == 0) {
            // only recalculate order totals if order totals are set and the count of items are greater than 0
            // if this is not catched the SW-Plugin BackendOrders will insert invalid data
            return;
        }
        $invoiceAmount = 0;
        $invoiceAmountNet = 0;
