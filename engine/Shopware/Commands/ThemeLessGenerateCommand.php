 ...
 */
class ThemeLessGenerateCommand extends ShopwareCommand
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this
            ->setName('sw:theme:less:generate')
            ->setDescription('Generates theme less files to css.')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($shops as $shop) {
            $output->writeln(sprintf('Generating less cache for shop "%s" ...', $shop->getName()));
            $timestamp = $compiler->getThemeTimestamp($shop);
            $compiler->compileLess($timestamp, $shop->getTemplate(), $shop);
        }
    }
}
