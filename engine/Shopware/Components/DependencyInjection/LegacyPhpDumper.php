 ...
 *
 * Modified PhpDumper for InitResource and AfterInitResource
 *
...
class LegacyPhpDumper extends Dumper
{
    ...
    private $reservedVariables = ['instance', 'class'];
...
EOF;
    ...
    {
        $allCalls = $calls = $behavior = [];

        foreach ($allInlinedDefinitions as $def) {
            $arguments = [$def->getArguments(), $def->getFactory(), $def->getProperties(), $def->getMethodCalls(), $def->getConfigurator()];
        ...
        foreach ($inlinedDefinitions as $def) {
            $this->getServiceCallsFromArguments([$def->getArguments(), $def->getFactory()], $calls, $isPreInstance, $cId);
            if ($def !== $definition) {
                $arguments = [$def->getProperties(), $def->getMethodCalls(), $def->getConfigurator()];
        ...
        foreach ($calls as $id => $callCount) {
            if ($id === 'service_container' || $id === $cId || isset($this->referenceVariables[$id])) {
                continue;
            }
            ...
            $reference = $behavior[$id] === ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE ? new Reference($id, $behavior[$id]) : null;
...
EOTXT;
        ...
        if ($this->inlineRequires && !$this->isHotPath($definition)) {
            ...
            foreach ($calls as $id => $callCount) {
                if ($id !== 'service_container' && $id !== $cId
                    && $behavior[$id] !== ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE
    ...
    private function addServiceInlinedDefinitions($id, Definition $definition, \SplObjectStorage $inlinedDefinitions, &$isSimpleInstance)
    {
        ...
        foreach ($inlinedDefinitions as $def) {
            ...
            if ($inlinedDefinitions[$def] <= 1 && !$def->getMethodCalls() && !$def->getProperties() && !$def->getConfigurator() && strpos($this->dumpValue($def->getClass()), '$') === false) {
                continue;
            }
            ...
            if (isset($inlinedDefinition[$definition]) && $this->hasReference($id, [$def->getArguments(), $def->getFactory()])) {
                throw new ServiceCircularReferenceException($id, [$id]);
            }

            $code .= $this->addNewInstance($def, '$' . $name, ' = ', $id);

            if (!$this->hasReference($id, [$def->getProperties(), $def->getMethodCalls(), $def->getConfigurator()], true)) {
    ...
    private function addServiceInlinedDefinitionsSetup($id, Definition $definition, \SplObjectStorage $inlinedDefinitions, $isSimpleInstance)
    {
        ...
        $code = '';
        foreach ($inlinedDefinitions as $def) {
            if ($definition === $def || !$this->hasReference($id, [$def->getProperties(), $def->getMethodCalls(), $def->getConfigurator()], true)) {
                continue;
            }
...
EOF;
        ...
        $constructorDefinitions = $this->getDefinitionsFromArguments([$definition->getArguments(), $definition->getFactory()]);
...
EOF;
     ...
     */
    private function getServiceCallsFromArguments(array $arguments, array &$calls, $isPreInstance, $callerId, array &$behavior = [], $step = 1)
    {
    ...
    private function getDefinitionsFromArguments(array $arguments, \SplObjectStorage $definitions = null)
    {
        ...
        foreach ($arguments as $argument) {
            if (is_array($argument)) {
                ...
                $this->getDefinitionsFromArguments([$argument->getFactory()], $definitions);
                ...
                $this->getDefinitionsFromArguments([$argument->getConfigurator()], $definitions);
     ...
     * @return bool
     */
    private function hasReference($id, array $arguments, $deep = false, array &$visited = [])
    {
        ...
        foreach ($arguments as $argument) {
            ...
            if ($this->hasReference($id, [$service->getArguments(), $service->getFactory(), $service->getProperties(), $service->getMethodCalls(), $service->getConfigurator()], $deep, $visited)) {
                return true;
            }
        }
    ...
    private function dumpValue($value, $interpolate = true)
    {
        if (is_array($value)) {
            ...
            $scope = [$this->definitionVariables, $this->referenceVariables, $this->variableCount];
