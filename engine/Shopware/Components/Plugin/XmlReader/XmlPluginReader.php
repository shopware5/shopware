...
class XmlPluginReader extends XmlReaderBase
{
    ...
    protected function parseFile(\DOMDocument $xml)
    {
        ...
        $info['label'] = self::parseTranslatableNodeList(
            $xpath->query('//plugin/label')
        );

        $info['description'] = self::parseTranslatableNodeList(
            $xpath->query('//plugin/description')
        );
