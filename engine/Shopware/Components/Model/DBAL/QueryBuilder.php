...
class QueryBuilder
{
    ...
    /**
     * Helper proeprty to use the straight join
     * @var bool
     */
    private $useStraightJoin = false;


    /**
     * Helper proeprty to use the SQL_NO_CACHE
     * @var bool
     */
    private $useSqlNoCache = false;

    /**
     * Helper property to set an force index condition
     * @var null
     */
    private $forceIndex = null;

    /**
     * Helper property to set the DISTINCT parameter
     * @var bool
     */
    ...
    private function getSQLForSelect()
    {
        ...
        $query = $query . ' ' . implode(', ', $this->sqlParts['select']) . ' FROM ';
        ...
        foreach ($this->sqlParts['from'] as $from) {
            if ($this->forceIndex !== null) {
                $fromClause = $from['table'] . ' ' . $from['alias'] . ' FORCE INDEX (' . $this->forceIndex . ') ';
        ...
        $joins = array();
        foreach ($this->sqlParts['join'] as $key => $join) {
            foreach($join as $joinTable) {
                $joins[] = ' ' . strtoupper($joinTable['joinType'])
                    . ' JOIN ' . $joinTable['joinTable'] . ' ' . $joinTable['joinAlias']
                    . ' ON ' . ((string) $joinTable['joinCondition']);
            }
        }
    ...
    /**
     * Enables or disables a STRAIGHT_JOIN condition for sql queries.
     * @param boolean $useStraightJoin
     */
    public function straightJoin($useStraightJoin = true)
    {
        $this->useStraightJoin = $useStraightJoin;
    }

    /**
     * Enables or disables the SQL_NO_CACHE condition of the sql queries.
     * @param boolean $useNoCache
     */
    public function sqlNoCache($useNoCache = true)
    {
        $this->useSqlNoCache = $useNoCache;
    }

    /**
     * Helper function to enable a FORCE INDEX condition.
     * The passed parameter has to be contains the index name.
     * To disable the forceIndex set the indexName parameter to null.
    ...
    /**
     * Sets the DISTINCT parameter for sql statements.
     */
    ...
    /**
     * Returns true if the distinct parameter is set to true.
     * @return boolean
     */
