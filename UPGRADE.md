...
* New PaymentMethods core plugin and API:
 * Payment methods custom logic should be implemented by extending the `ShopwarePlugin\PaymentMethods\Components\BasePaymentMethod` class and registered using the `Shopware_Modules_Admin_InitiatePaymentClass_AddClass` event
 * Core `paymentmeans` classes are deprecated and must be used only with the provided `ShopwarePlugin\PaymentMethods\Components\DebitPaymentMethod` and `ShopwarePlugin\PaymentMethods\Components\GenericPaymentMethod` wrapper classes
