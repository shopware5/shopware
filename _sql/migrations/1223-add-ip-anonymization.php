...
class Migrations_Migration1223 extends Shopware\Components\Migrations\AbstractMigration
{
    public function up($modus)
    {
        ...
        $sql = "INSERT INTO `s_core_config_elements` (`form_id`, `name`, `value`, `label`, `description`, `type`, `required`, `position`, `scope`)
                VALUES (@privacyFormId, 'anonymizeIp', '" . serialize(true) . "', 'Kunden IPs anonymisieren', 'Entfernt die letzten zwei Blöcke einer IPv4, resp. 3 Blöcke einer IPv6 Adresse in Statistiken und Bestellungen um rechtlichen Rahmenbedingungen einzuhalten.', 'boolean', 0, 0, 0);";
        $this->addSql($sql);
        ...
        $sql = "INSERT INTO `s_core_config_element_translations` (`element_id`, `locale_id`, `label`, `description`)
                VALUES (@elementId, '2', 'Anonymize customer\\'s IPs', 'Removes the last two blocks of IPv4 and 3 blocks of IPv6 addresses in statistics and orders to comply with privacy laws.');";
        $this->addSql($sql);
    }
}
