...
class Migrations_Migration943 extends Shopware\Components\Migrations\AbstractMigration
{
    public function up($modus)
    {
        $sql = <<<'EOD'
            ...
            VALUES (144, 'manufacturerProductBoxLayout', 's:5:"basic";', 'Produktlayout im Herstellerlisting', '', 'product-box-layout-select', 0, 0, 1);
            SET @elementId = (SELECT `id` FROM `s_core_config_elements` WHERE `form_id`= 144 AND `name`="manufacturerProductBoxLayout" LIMIT 1);
            INSERT IGNORE INTO `s_core_config_element_translations` (`label`, `description`, `locale_id`, `element_id`)
            VALUES ('Manufacturer page product layout', '', 2, @elementId);
EOD;
        $this->addSql($sql);
    }
}
