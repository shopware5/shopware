		...
		<div class="blogbox_content">
            ...
            {if $sArticle.preview.thumbNails.1}
                <div class="blog_picture">
                    ...
                    <a href="{url controller=blog action=detail sCategory=$sArticle.categoryId blogArticle=$sArticle.id}" title="{$sArticle.title}" style="background: url({link file=$sArticle.preview.thumbNails.1}) no-repeat center center;" class="main_image">
                    </a>
                    {/block}
                </div>
            {/if}
			...
			<div class="blog_tags">
				{if $sArticle.tags|@count > 1}
                    ...
                    {/foreach}
				{/if}					
				<a href="{url controller=blog action=detail sCategory=$sArticle.categoryId blogArticle=$sArticle.id}" title="{$sArticle.title}" class="button-right small_right right">{se name="BlogLinkMore"}{/se}</a>
			</div>
			{/block}
		</div>
	{/block}
