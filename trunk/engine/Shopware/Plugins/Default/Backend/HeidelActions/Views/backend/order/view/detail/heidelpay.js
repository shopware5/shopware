...
Ext.define('Shopware.apps.Order.view.detail.Heidelpay', {
    ...
    createIFrameContainer: function() {
        ...
        var myFrame = Ext.create('Ext.panel.Panel', {
          ...
          height: 525,
          top: 0,
          html: '<iframe src="backend/HeidelBooking?uid='+me.record.getAttributes().first().get('attribute2')+'&&cid='+me.record.getAttributes().first().get('attribute5')+'" frameborder="0"></iframe>',
          //renderTo: Ext.getBody()
        });


	if (me.record && me.record.getAttributes() && me.record.getAttributes().first()) {
    		if (me.record.getAttributes().first().get('attribute6') === 'HEIDELPAY') {
       		 return myFrame;
    		 }
	}
