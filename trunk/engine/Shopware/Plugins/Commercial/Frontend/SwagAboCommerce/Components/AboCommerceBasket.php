...
/**
 * Shopware SwagAboCommerce Plugin - Basket Component
 *
 * @category  Shopware
 ...
 * @Shopware\noEncryption
 */
class Shopware_Components_AboCommerceBasket extends Enlight_Class implements Enlight_Hook
{
    /**
     ...
     * 2. The article and the variant of the passed order number has to been activated
     ...
     *     for the current customer session  and the passed quantity for the passed article.
     ...
     *
     * @return array Result of the add article to basket process.
     */
    ...
    {
        if (!$item instanceof \Shopware\Models\Order\Basket) {
            $item = $this->getItem($item, \Doctrine\ORM\AbstractQuery::HYDRATE_OBJECT);
        }
    ...
    public function getPriceForQuantity($prices, $quantity, $variant, $parameter = array())
    {
        ...
        foreach ($prices as $price) {
            if (!is_numeric($price->getTo())) {
                $currentPrice = $price;
                break;
            } elseif ($quantity >= $price->getFrom() && $quantity <= $price->getTo()) {
                $currentPrice = $price;
                break;
            }
        }
