...
{block name='frontend_listing_box_article_price'}
	<p class="points">
		<span class="{if $sBonusSystem.points.remaining >= $sArticle.required_points}enough{else}not_enough{/if}">{s namespace="frontend/bonus_system" name="ForXBonusPoints"}f&uuml;r <strong>{$sArticle.required_points} Punkte</strong>{/s}</span>
