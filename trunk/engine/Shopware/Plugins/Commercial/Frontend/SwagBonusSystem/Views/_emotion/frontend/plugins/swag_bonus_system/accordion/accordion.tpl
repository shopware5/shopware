				...
				<li {if $sArticle@first}class="active"{/if}>
					...
					<div class="detail">
						<a href="{$sArticle.linkDetails}" title="{$sArticle.articleName}">
							{$sArticle.articleName|truncate:18:"...":true}
						</a>
						<div class="points">
							{s namespace="frontend/bonus_system/accordion" name="ForXBonusPoints"}f&uuml;r <strong>{$sArticle.required_points}</strong> Punkte{/s}
						</div>
					</div>
