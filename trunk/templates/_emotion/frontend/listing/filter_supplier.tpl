{extends file='parent:frontend/listing/filter_supplier.tpl'}
		...
		{block name="frontend_listing_filter_supplier_each"}
			<div class="slideContainer">
				<ul{if $sSuppliers|@count > 5} class="overflow"{/if}>
				    ...
				    {else}
				        <li class="{if $sSupplierInfo.name eq $supplier.name}active{/if} {if $smarty.foreach.supplier.last}last{/if}" id="n{$supKey+1}">
				        {if $sSupplierInfo.name eq $supplier.name}
				        	{$supplier.name} ({$supplier.countSuppliers})
				        	{else} 
				        		<a href="{$supplier.link}" title="{$supplier.name}">{$supplier.name} ({$supplier.countSuppliers})</a>
				        	{/if}
				        </li>
				    {/if}
					{/foreach}
