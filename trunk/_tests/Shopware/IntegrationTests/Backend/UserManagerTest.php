<?php
/**
 * Shopware
 *
 * LICENSE
 *
 * Available through the world-wide-web at this URL:
 * http://shopware.de/license
 * If you did not receive a copy of the license and are unable to
 * obtain it through the world-wide-web, please send an email
 * to license@shopware.de so we can send you a copy immediately.
 *
 * @category   Shopware
 * @package    Shopware_IntegrationTests
 * @subpackage Backend
 * @copyright  Copyright (c) 2012, shopware AG (http://www.shopware.de)
 * @license    http://shopware.de/license
 * @version    $Id$
 * @author:    st.hamann
 * @author     $Author$
 */

/**
 * Test case
 *
 * @category   Shopware
 * @package    Shopware_IntegrationTests
 * @subpackage Backend
 * @copyright  Copyright (c) 2012, shopware AG (http://www.shopware.de)
 * @license    http://enlight.de/license
 */
class Shopware_IntegrationTests_Backend_UserManagerTest extends Shopware_IntegrationTests_Backend_LoginTest
{
   /**
    * Test that the creation of new backend users is working
    */
   public function testBasicUser()
   {
       $this->login();
       $this->openUserManager();

       // Add new user
       $this->click('css=div[id^=usermanager-user-list] button[data-action=addUser]');
       $this->waitForElementPresent("css=div[id^=usermanager-user-create] input[name=username]");
   }


    /**
     * Initial opening user manager
     */
    private function openUserManager()
    {
        sleep(1);
       $this->open('backend/?app=UserManager');
       // Wait until default layer is loaded
       $this->waitForElementPresent("css=.x-window-header-text");
       $this->waitForElementPresent('css=div[id^=usermanager-user-list] button[data-action=addUser]');
    }
}